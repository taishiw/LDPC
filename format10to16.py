...
for i in range(len(g)):
    if x[i]>=0.0:
        f.write(hex(int(x[i]*(16**DECIMAL_WIDTH)))+'\n')
    else :
        f.write(hex(int(16**WIDTH+x[i]*(16**DECIMAL_WIDTH)))+'\n')
for j in range (len(k)):
    y[j]=float(k[j])
#print (y)
for i in range(len(k)):
    if y[i]>=0.0:
        h.write(hex(int(y[i]*(16**DECIMAL_WIDTH)))+'\n')
    else :
        h.write(hex(int(16**WIDTH+y[i]*(16**DECIMAL_WIDTH)))+'\n')
...
for i in range(len(m)):
    if z[i]>=0.0:
        l.write(hex(int(z[i]*(16**DECIMAL_WIDTH)))+'\n')
    else :
        l.write(hex(int(16**WIDTH+z[i]*(16**DECIMAL_WIDTH)))+'\n')
